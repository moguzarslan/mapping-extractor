"""The orchestration around a requirement extraction: which documents, how many
runs, where the results go and how they are evaluated.
...
from service.evaluator_service import (
    evaluate,
    write_average_report,
    write_average_type_breakdown_report,
)
from service.prompt_service import save_requirements
...
class RequirementExtractionRunner:
    ...
    def __init__(self, version: RequirementVersion, runs: int = DEFAULT_RUNS,
    ...
    @classmethod
    def from_env(cls) -> "RequirementExtractionRunner":
    ...
    def run(self) -> None:
        print(f"Requirement extraction '{self.version.name}' — {self.version.description}")
    ...
    def run_document(self, file_name: str) -> None:
        folder = PROJECT_ROOT / "resource" / "docs" / file_name
        document = folder / f"{file_name}.pdf"

        # The combined GT workbook holds both a Requirements sheet and an optional
        # Concepts sheet (Concept ID -> Description), see
        # service/evaluator_service.load_ground_truth_concepts.
        gt_path = (PROJECT_ROOT / "resource" / "groundTruths" / "requirement"
                   / f"{file_name}_ground_truth_requirement.xlsx")
        has_gt = gt_path.exists()
        if not has_gt:
            print(f"No requirement ground truth found for '{file_name}', skipping evaluation.")
        ...
        for run in range(1, self.runs + 1):
            ...
            result = strategy.extract(str(document), str(folder))

            # The two final artifacts the evaluation consumes.
            requirements_json_path, concepts_json_path = save_requirements(
                result.requirements, result.concepts,
                output_file_name=file_name,
                output_dir=str(self.extraction_dir(file_name) / run_label),
            )
            print(f"Requirements ({strategy.label}) saved: {requirements_json_path}")
            print(f"Concepts saved: {concepts_json_path}")

            if has_gt:
                eval_output_path = (self.evaluation_dir(file_name) / run_label
                                    / f"{file_name}_req_eval.xlsx")
                reports.append(evaluate(
                    str(gt_path), requirements_json_path, str(eval_output_path),
                    llm_concepts_path=concepts_json_path))
                print(f"Requirement evaluation saved: {eval_output_path}")
    ...
    def write_averages(self, file_name: str, reports: list) -> None:
        ...
        avg_output_path = avg_dir / f"{file_name}_req_eval_avg.xlsx"
        write_average_report(reports, str(avg_output_path))
        print(f"Average evaluation saved: {avg_output_path}")

        avg_by_type_output_path = avg_dir / f"{file_name}_req_eval_avg_by_type.xlsx"
